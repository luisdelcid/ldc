...
if(!class_exists(__NAMESPACE__ . '\LDC') and defined('ABSPATH')){
	class LDC {
		...
		public static function _wp_enqueue_scripts(){
			...
			if($enqueue_scripts){
				$handle = str_replace('_', '-', self::PREFIX . 'scripts');
		...
		public static function build_update_checker(...$args){
			$remote_lib = use_plugin_update_checker();
		...
		public static function file_get_html(...$args){
			$remote_lib = use_simple_html_dom();
			if(is_wp_error($remote_lib)){
				return $remote_lib;
			}
		...
		public static function hide_uploads_subdir($subdir = '', $file = ''){
			...
			$path = plugin_dir_path(__FILE__) . 'readfile.js';
		...
		public static function serializable_closure(...$args){
			$remote_lib = use_serializable_closures();
			if(is_wp_error($remote_lib)){
				return $remote_lib;
			}
		...
		public static function tgmpa($plugins = [], $config = []){
			...
			$remote_lib = use_tgm_plugin_activation();
			if(is_wp_error($remote_lib)){
				return;
			}
			$args = [
